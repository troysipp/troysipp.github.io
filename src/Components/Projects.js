import React, { Component } from "react";
import { Link } from "react-router-dom";

import "./Projects.css";

class Projects extends Component {
  render() {
    return (
      <div className="project-list">
        <div className="project">
          <a href="http://social-network.surge.sh/home">JustForFriends</a>
          <a href="https://github.com/troysipp/social-network-frontend">
            Github, Jan 17
          </a>
        </div>
        <div className="project">
          <a href="http://home-finder.surge.sh/homes">Home Finder</a>
          <a href="https://github.com/sloan-holzman/home-app-frontend">
            Github, Nov 2017
          </a>
        </div>

        <div className="project">
          <a href="https://recipe-eat.herokuapp.com/recipes">recipeat</a>
          <a href="https://github.com/troysipp/Recipe-Privatization">
            Github, Oct 2017
          </a>
        </div>
        <div className="project">
          <a href="https://troysipp.github.io/tower-of-hanoi/">
            Tower of Hanoi
          </a>
          <a href="https://github.com/troysipp/tower-of-hanoi">
            Github, Oct 2017
          </a>
        </div>
      </div>
    );
  }
}

export default Projects;
